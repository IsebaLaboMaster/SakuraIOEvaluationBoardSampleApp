...
//〇動作
//ボタン操作は以下の通りになります。
//SW1:手番の横軸座標選択ボタン。押下でカウントアップし、abcdefgh9と変化します。9の次はaになります。
//SW2:手番の縦軸座標選択ボタン。押下でカウントアップし、1～9まで変化します。9の次は1になります。
//SW3:指し手決定ボタン。自分の手番の時で横軸(a～h) 縦軸(1～8)の時、盤に置ける有効な手だったらそこに石を置きます。
//                  盤の座標を超えた場合(X,Y座標がどちらか9以上)、このSWは無効です。
//SW4:設定用ボタン。(横軸,縦軸)を(9,9)に指定して押下するとsakura.io検証用ボード側を黒番にしてリセットします。
//                  (横軸,縦軸)を(9,8)に指定して押下するとsakura.io検証用ボード側を白番にしてリセットします。
//                  但し、使用sakura.ioポイント数は初期化されません。
//                  (横軸,縦軸)を(9,1)に指定して押下するとAS-289R2から対局情報を出力させることができます。
//                  (但し、評価ボードの拡張IOとAS-289R2を接続する必要有り)
//                  盤内の座標数値(1,1)～(8,8)の時、このSWは無効です。
//SW5:LCD点灯スイッチ(LCD側(7側)にスライドすると手番に関わらず常時点灯します。6側にするとこの機能をOFFします)
//SW6:Web定期更新スイッチ(LCD側(7側)にスライドすると10秒に一回Web側へ現在の盤状態を送信します。6側にするとこの機能をOFFします)
//
...
public:
    ...
    void PrintOutAS289R2();//現在の盤状態をプリンタシールドから出力
    void AddRecord(int record);//棋譜記録配列に座標情報を追加していく
    char* GetXCoordinateName(int x);//X座標の数値からa～hに変換
    ...
    int Record[64];//100の位:石の色 10の位:X座標 1の位:Y座標
