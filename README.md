...
# ―プログラム使用上の注意―
**以下の3つのプログラムは通信モジュールの使用を考慮に入れていますので、sakura.ioポイントを消費します。**
...
但し、「gps\_xa1110_test.html」については以下の準備が必要になります。  

# ―「gps\_xa1110_test.html」について―
...
# ―サンプルWebアプリケーション使い方(bme280_test.html)―
...
※検証用ボードの角度ではなく「角度の変化量」に応じて動くので  
**検証用ボードの傾き＝MMDモデルの傾き**  
とはならないです。  

ヒント：検証用ボードは極力ゆっくり一定に動かしてみてください。あまりにも変な角度になった場合は「初期位置」ボタンを押下してください。
